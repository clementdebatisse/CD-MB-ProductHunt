<?php
function US_Slider_Block() {
    if(function_exists('render_block_core_block')){  
		wp_register_script( 'ewd-us-blocks-js', plugins_url( '../blocks/ewd-us-blocks.js', __FILE__ ), array( 'wp-blocks', 'wp-element', 'wp-components', 'wp-editor' ) );
		wp_register_style( 'ewd-us-blocks-css', plugins_url( '../blocks/ewd-us-blocks.css', __FILE__ ), array( 'wp-edit-blocks' ), filemtime( plugin_dir_path( __FILE__ ) . '../blocks/ewd-us-blocks.css' ) );
		register_block_type( 'ultimate-slider/ewd-us-slider-block', array(
			'attributes'      => array(
				'category' => array(
					'type' => 'string',
				),
...
function EWD_US_Display_Slider($atts, $content = null) {
	...
	//START HIDE FROM ALL, NOT JUST MOBILE
	if (in_array("title", $Hide_From_Slider)) {$Title_All_Hide_Class = 'ewd-us-all-hide';}
	else {$Title_All_Hide_Class = "";}
	if (in_array("body", $Hide_From_Slider)) {$Body_All_Hide_Class = 'ewd-us-all-hide';}
	else {$Body_All_Hide_Class = "";}
	if (in_array("buttons", $Hide_From_Slider)) {$Buttons_All_Hide_Class = 'ewd-us-all-hide';}
	else {$Buttons_All_Hide_Class = "";}
	if (in_array("arrows", $Hide_From_Slider)) {
		$Arrow_All_Hide_Class = 'ewd-us-all-hide';
		$Arrow_Div_All_Hide_Class = 'ewd-us-all-arrow-hide';
	}
	else {
		$Arrow_All_Hide_Class = "";
		$Arrow_Div_All_Hide_Class = "";
	}
	if (in_array("thumbnails", $Hide_From_Slider)) {$Thumbnails_All_Hide_Class = 'ewd-us-all-hide';}
	else {$Thumbnails_All_Hide_Class = "";}
	//END HIDE FROM ALL
	...
	$return_string .= '<div class="slider' . ( ( ($slide_indicators == "" && $Slide_Indicators == "SideThumbnails") || $slide_indicators == "Side Thumbnails" ) ? ' side-thumbnails' : '' ) . '">';
	if($timer_bar != 'No'){
	...
	$return_string .= '<ul class="slider-window">';
		...
		while ($Slider_Query->have_posts()) : $Slider_Query->the_post();
			...
			elseif ($slider_type == "urp") {
				...
				$Review_ID = $post->ID;
				if (!isset($Buttons) or !is_array($Buttons)) {$Buttons = array();}
			}
			else {
			...
			if (isset($Image_Type) and $Image_Type == "youtube_video") {
			...
			if (isset($Max_Title_Chars) and $Max_Title_Chars != "") {$Title = substr($Title, 0, $Max_Title_Chars);}
			if (isset($Max_Body_Chars) and $Max_Body_Chars != "") {$Content = substr($Content, 0, $Max_Body_Chars);}
			...
			$return_string .= '<li class="ewd-slide ' . $carouselClass . $Hidden_Class . $Video_Background_Class . $Lightbox_Class . '" id="ewd-slide-' . $Slide_Counter . '" ' . $Lightbox_Data . '>';
				...
				$return_string .= '<div class="slideText">';
					$return_string .= '<div class="slideTitle ' . $Title_Mobile_Class . ' ' . $Title_All_Hide_Class . '">' . $Title . '</div>';
					$return_string .= '<div class="clear"></div>';
					$return_string .= '<div class="slideExcerpt ' . $Body_Mobile_Class . ' ' . $Body_All_Hide_Class . '">' . $Content . '</div>';
					$return_string .= '<div class="clear"></div>';
					$return_string .= '<ul class="slideButtons">';
						foreach ($Buttons as $Button) {
							...
							$return_string .= '<li>';
							$return_string .= "<a class='ewd-us-slide-button " . $Buttons_Mobile_Class . " " . $Buttons_All_Hide_Class . "' href='" . $Link . "' " . $Target_Text . ">" . strtoupper($Button['Text']) . "</a>";
							$return_string .= '</li>';
						}
					$return_string .= '</ul>';
	...
	$return_string .= ' <div class="nav-arrow" id="left"><div class="ewd-us-arrow-div ' . $Background_Class . ' ' . $Arrow_Div_Mobile_Class . ' ' . $Arrow_Div_All_Hide_Class . '"><div class="ewd-slider-icon us-arrow ' . $Arrow_Mobile_Class . ' ' . $Arrow_All_Hide_Class . '" id="left">' . $Selected_Arrow . '</div></div></div> <!-- nav arrow -->';
	$return_string .= ' <div class="nav-arrow" id="right"><div class="ewd-us-arrow-div ' . $Background_Class . ' ' . $Arrow_Div_Mobile_Class . ' ' . $Arrow_Div_All_Hide_Class . '"><div class="ewd-slider-icon us-arrow ' . $Arrow_Mobile_Class . ' ' . $Arrow_All_Hide_Class . '"  id="right">' . chr(ord($Selected_Arrow)+1) . '</div></div></div> <!-- nav arrow -->';
	$return_string .= '</div> <!-- slider -->';
	if ( ($slide_indicators == "" && $Slide_Indicators == "Dots") || $slide_indicators == "Dots" ) {$return_string .= '<div class="ewd-slider-control" id=""><div class="ewd-slider-control-button-list"></div></div>';}
	if ( ($slide_indicators == "" && $Slide_Indicators == "Thumbnails") || $slide_indicators == "Thumbnails" ) {
	...
	if ( ($slide_indicators == "" && $Slide_Indicators == "SideThumbnails") || $slide_indicators == "Side Thumbnails" ) {
		$return_string .= '<div class="ewd-slider-control"></div>';
		$return_string .= '<div class="ewd-slider-control-side-thumbnails">';
			foreach ($Slide_Previews as $Slide_Counter => $Image_URL) {
				$return_string .= '<div class="ewd-slider-control-thumbnail-side ' . $Thumbnails_Mobile_Class . '" data-slidenumber="' . $Slide_Counter . '">';
				$return_string .= '<img src="' . $Image_URL . '" class="ewd-slider-control-thumbnail-img" />';
				$return_string .= '</div>';
			}
		$return_string .= "</div>";
